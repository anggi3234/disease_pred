...
def calculate_risk_scores(features):
    ...
    metabolic_risk = (
        0.3 * (features['bmi'] - 18.5) / (30 - 18.5) +  # BMI contribution
    ...
    cv_risk = (
        ...
        0.25 * (1 - features['diet_quality']) +  # Diet contribution
        0.2 * (1 - min(features['met_hours'] / 40, 1)) +  # Activity contribution
        0.15 * features['stress_score'] +  # Stress contribution
        0.15 * features['smoking_risk']  # Smoking contribution
    )
    ...
    recovery_risk = (
        0.4 * (1 - features['sleep_score']) +  # Sleep contribution
        0.3 * features['stress_score'] +  # Stress contribution
        0.3 * (1 - min(features['met_hours'] / 40, 1))  # Activity contribution
    )
...
def display_results(risk_scores, recommendations):
    ...
    with col1:
        st.metric("Metabolic Health Risk", f"{risk_scores['metabolic_health']*100:.1f}%")
    with col2:
        st.metric("Cardiovascular Risk", f"{risk_scores['cardiovascular']*100:.1f}%")
    with col3:
        st.metric("Sleep & Recovery Risk", f"{risk_scores['sleep_recovery']*100:.1f}%")
