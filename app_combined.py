...
symptom_scale = {
    ...
    "id": {
        "Never": "Tidak pernah", "Sometimes": "Kadang-kadang", "Often": "Sering", "Always": "Selalu"
    }
}
...
def physical_activity_section():
    ...
    with col1:
        exercise_freq = map_selectbox(T['exercise_freq'], exercise_freq_map, key="exercise_freq")
    
    with col2:
        duration = map_selectbox(T['exercise_duration'], duration_map, key="exercise_duration")
        intensity = map_selectbox(T['exercise_intensity'], intensity_map, key="exercise_intensity")
...
def lifestyle_section():
    ...
    with col1:
        ...
        total_cholesterol = map_selectbox(T['cholesterol_level'], cholesterol_map, key="cholesterol_level")
        blood_pressure_medication = map_selectbox(T['bp_meds'], bp_map, key="bp_meds")
    
    with col2:
        smoking = map_selectbox(T['smoking_status'], smoking_map, key="smoking_status")
        alcohol = map_selectbox(T['alcohol_use'], alcohol_map, key="alcohol_use")
        hba1c = map_selectbox(T['hba1c_label'], hba1c_map, key="hba1c_label")
        fasting_glucose = map_selectbox(T['fasting_glucose'], glucose_map, key="fasting_glucose")
...
def health_conditions_section():
    ...
    symptoms = {}
    with col1:
        symptoms["fatigue"] = map_selectbox(T['symptom_fatigue'], symptom_scale, key="fatigue")
        symptoms["joint_pain"] = map_selectbox(T['symptom_joint_pain'], symptom_scale, key="joint_pain")
        symptoms["digestive"] = map_selectbox(T['symptom_digestive'], symptom_scale, key="digestive")
        symptoms["skin_issues"] = map_selectbox(T['symptom_skin_issues'], symptom_scale, key="skin_issues")

    with col2:
        symptoms["headaches"] = map_selectbox(T['symptom_headaches'], symptom_scale, key="headaches")
        symptoms["mood"] = map_selectbox(T['symptom_mood'], symptom_scale, key="mood")
        symptoms["cognitive"] = map_selectbox(T['symptom_cognitive'], symptom_scale, key="cognitive")
        symptoms["sleep_issues"] = map_selectbox(T['symptom_sleep'], symptom_scale, key="sleep_issues")
